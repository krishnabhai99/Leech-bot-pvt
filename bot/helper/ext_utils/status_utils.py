...
def get_readable_message(sid: int, is_user: bool, page_no: int=1, status : str='All', page_step: int=1):
    ...
    for index, task in enumerate(tasks[start_position:STATUS_LIMIT + start_position], start=1):
        tstatus = task.status()
        msg += f'<blockquote><b>{index+start_position}.</b> <code>{escape(str(task.name())) or "N/A"}</code></blockquote>'
        ...
        if tstatus not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_METADATA, MirrorStatus.STATUS_SUBSYNC]:
            msg += (f'\n<blockquote><b>{get_progress_bar_string(task.progress())}</b></blockquote>'
            ...
            msg += (f'\n<b>Size:</b> {task.size()}'
                    f'\n<b>Elapsed:</b> {task.elapsed() or "~"}')
        msg += f'{ext_msg}\n<blockquote><code>/{BotCommands.CancelTaskCommand} {task.gid()}</code></blockquote>\n\n'
    ...
    msg += ('▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n'
            f'<blockquote><b>CPU:</b> {cpu_percent()}% <b>| RAM:</b> {virtual_memory().percent}% <b>| FREE:</b> {get_readable_file_size(disk_usage(config_dict["DOWNLOAD_DIR"]).free)}\n'
            f'<b>IN:</b> {get_readable_file_size(net_io_counters().bytes_recv)}<b> | OUT:</b> {get_readable_file_size(net_io_counters().bytes_sent)}\n'
            f'<b>DL:</b> {get_readable_file_size(dl_speed)}/s<b> | UL:</b> {get_readable_file_size(up_speed)}/s <b>|</b> {get_readable_time(time() - botStartTime)}</blockquote>')
