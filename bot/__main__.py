...
async def start(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if data == user_dict.get('session_token'):
            ...
            text, image = 'This seassion has been expired!', config_dict['IMAGE_UNAUTH']
    elif await CustomFilters.authorized(client, message):
        text = f'Bot ready to use, type /{BotCommands.HelpCommand} to get a list of available commands'
...
async def restart(_, message: Message):
    ...
    if hrestart:
        ...
        _, msg = await gather(kill_route(), sendMessage('<i>Restarting with normal mode...</i>', message))
        ...
        proc1 = await create_subprocess_exec('pkill', '-9', '-f', f'gunicorn|{ARIA_NAME}|{QBIT_NAME}|{FFMPEG_NAME}|rclone|java|alass')
