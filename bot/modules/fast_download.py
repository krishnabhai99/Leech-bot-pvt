...
class FastDL(TaskListener):
    ...
    async def newEvent(self):
        ...
        rjson = await cmd_exec(['rclone', 'backend', 'addurl', '--config', 'rclone.conf', upload_path, self.link])
        if rjson[2] != 0:
        ...
        if rjson.get('message') == 'Saving':
            ...
            typee = await cmd_exec(['rclone', 'lsjson', '--fast-list', '--stat', '--no-modtime', '--config', 'rclone.conf', f'{upload_path}/{name}'])
            res = loads(typee[0])
