...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    if not data:
        ...
        text = ('<blockquote><b>USER SETTINGS</b></blockquote>\n\n'
                f'<blockquote>Settings For: <b>{from_user.mention}</b></blockquote>\n'
                f'<blockquote expandable>{status_user}' 
                f'{premium_left}'
                f'{daily_limit}'
                f'<b>Leech Type:</b>{ltype}</b>\n' 
                f'<b>As Group:</b>{mediagroup}</b>\n'
                f'<b>Thumbnail:</b>{thumbmsg}</b>\n'
                f'<b>DM Mode:</b>{sendpm}</b>\n'
                f'<b>SS Mode:</b>{sendss}</b>\n'
                f'<b>RClone:</b>{rccmsg}</b>\n'
                f'<b>Caption:</b>{capmode.upper()}</b>\n'
                f'<b>Prename:</b>{premsg}</b>\n'
                f'<b>Sufname:</b>{sufmsg}</b>\n'
                f'<b>Remname:</b>{rmmsg}</b>\n'
                f'<b>Metadata:</b>{metadata}</b>\n'
                f'<b>UserDump/Tds:</b> {dumpch}\n'
                f'<b>Custom GDrive:</b>{gdxmsg}</b>\n'
                f'<b>Custom Caption:</b>{custom_cap}</b>\n'
                f'<b>Session String:</b>{sesmsg}</b>\n'
                f'<b>Default Upload:</b>{du}</b>\n'
                f'<b>YT-DLP Options:</b> {yto}\n')
        text += f'<i>Leech Split Size ~ {get_readable_file_size(config_dict["LEECH_SPLIT_SIZE"])}</i></blockquote>'
        ...
        text = ('<blockquote><b>CAPTION SETTINGS</b></blockquote>\n'
        ...
        text = ('<blockquote><b>RCLONE SETTINGS</b></blockquote>\n'
        ...
        text = ('<blockquote><b>User GDRIVE SETTING</b></blockquote>\n'
        ...
        text = ('<blockquote><b>ZIP MODE SETTINGS</b>\n⁍ <b>Folders/Default:</b> Zip file/folder\n</blockquote>'
...
async def set_thumb(_, message: Message, query: CallbackQuery):
    ...
    handler_dict[user_id] = False
    msg = await sendMessage('<blockquote><i>Processing, please wait...</i></blockquote>', message)
