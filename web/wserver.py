...
<head>
    ...
    <link rel="icon" href="https://telegra.ph/file/6fb7da7f5298af43b46e7.png" type="image/jpg">
...
def re_verfiy(paused, resumed, client, hash_id):
    ...
    while True:
        ...
        try:
            ...
            LOGGER.error("%s Errored in reverification paused!", e)
        try:
            client.torrents_file_priority(torrent_hash=hash_id, file_ids=resumed, priority=1)
        except NotFound404Error as e:
            raise NotFound404Error from e
        except Exception as e:
            LOGGER.error("%s Errored in reverification resumed!", e)
        k += 1
        if k > 5:
            return False
    LOGGER.info("Verified! Hash: %s", hash_id)
    return True
...
def set_priority(id_):
    ...
    if len(id_) > 20:
        ...
        if res == "OK":
            LOGGER.info("Verified! Gid: %s", id_)
        else:
            LOGGER.info("Verification Failed! Report! Gid: %s", id_)
